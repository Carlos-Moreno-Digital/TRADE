"""Gauntlet runner for VRPArb (Phase 6).

Token rule: only the final verdict table on stdout. No tensors, no
dataframes.
"""
...
from trade.research.strategies.vrp_arb import VRPArb
...
def main() -> int:
    ...
    console.print(Panel.fit(
        f"[bold magenta]VRPArb Gauntlet[/bold magenta]\n"
        f"symbol={SYM} bars={N_BARS} combos={n_combos}\n"
        f"GARCH(1,1) frozen alpha={0.10} beta={0.85}",
        title="Phase 6 — Volatility Risk Premium",
        border_style="magenta",
    ))

    # Vanilla sanity (one line)
    ...
    # Verdict table
    console.print()
    t = Table(title=f"VRPArb Gauntlet — {SYM}")
    t.add_column("Gate")
    t.add_column("Metric")
    t.add_column("Result")
    t.add_row(
