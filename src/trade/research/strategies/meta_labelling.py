...
def _triple_barrier_label(
    ...
    """1 if TP hit first, 0 otherwise (SL or timeout). Pessimistic
    ordering: same-bar SL+TP -> 0.
    """
    n = len(high)
...
class MetaLabellingStrategy:
    ...
    default_params: dict[str, Any] = {
        ...
        "train_fraction": 0.4,
        "meta_threshold": 0.45,  # below 0.5 because the base rate of
                                  # TP-first labels on this data is ~30%
        "rf_n_estimators": 50,
        "rf_max_depth": 5,
    ...
    def _train_meta(
        ...
        for s in train_signals.itertuples():
            t0 = int(s.bar_idx) - 1  # back to 0-based
            ...
            label = _triple_barrier_label(
                t0, s.side, float(s.sl), float(s.tp),
                high, low, max_holding,
            )
            row = list(f.values) + [1.0 if s.side == "buy" else 0.0]
            rows.append(row)
            labels.append(label)
        if len(rows) < 20:
            return None  # too few labels to train
    ...
    def _walk_filtered(
        ...
        # Index signals by their entry bar (1-based -> 0-based)
    ...
    def _run(
        ...
        rf = self._train_meta(
            train_signals, feats, high, low, max_holding, params,
        )
